  ...
  workflow_dispatch:
    inputs:
      ENV_NAME:
        ...
        type: string
      JUMPSERVER_HOST:
        description: 'WireGuard VM public IP or DNS (SSH reachable)'
        required: true
        type: string
      SSH_PRIVATE_KEY:
        description: 'Name of the repo secret holding the WG VM SSH private key'
        required: true
        type: string
  ...
  onboard:
    ...
    steps:
      ...
      - name: Write SSH private key
        run: |
          mkdir -p ~/.ssh
          echo "${{ secrets[inputs.SSH_PRIVATE_KEY] }}" > ~/.ssh/jumpserver_key
          chmod 600 ~/.ssh/jumpserver_key
          if [ ! -s ~/.ssh/jumpserver_key ]; then
            echo "ERROR: SSH key secret '${{ inputs.SSH_PRIVATE_KEY }}' is empty"
            exit 1
          fi
      ...
      - name: Run WireGuard onboarding
        ...
        run: |
          ...
          .github/workflows/scripts/wg-onboard.sh \
            --env "${{ inputs.ENV_NAME }}" \
